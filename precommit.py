...
def main() -> int:
    ...
    if Step.MYPY in selects and Step.MYPY not in skips:
        print("Mypy'ing...")
        # fmt: off
        mypy_targets = ["schemathesis_x_contracts", "tests"]
    ...
    if Step.PYLINT in selects and Step.PYLINT not in skips:
        # fmt: off
        print("Pylint'ing...")
        pylint_targets = ["schemathesis_x_contracts"]
        subprocess.check_call(
    ...
    if Step.PYDOCSTYLE in selects and Step.PYDOCSTYLE not in skips:
        ...
        subprocess.check_call(
            ["pydocstyle", "schemathesis_x_contracts"], cwd=str(repo_root)
        )
    else:
    ...
    if Step.TEST in selects and Step.TEST not in skips:
        ...
        subprocess.check_call(
            [
                "coverage", "run",
                "--source", "schemathesis_x_contracts",
                "-m", "unittest", "discover"
            ],
            cwd=str(repo_root),
            env=env,
    ...
    if Step.DOCTEST in selects and Step.DOCTEST not in skips:
        ...
        print("Doctesting...")
        for pth in (repo_root / "schemathesis_x_contracts").glob("**/*.py"):
    ...
    if (
        ...
        print(
            "Checking that schemathesis_x_contracts/__init__.py and "
            "setup.py coincide..."
        )
        subprocess.check_call([sys.executable, "check_init_and_setup_coincide.py"])
    else:
        print(
            "Skipped checking that schemathesis_x_contracts/__init__.py and "
            "setup.py coincide."
        )
